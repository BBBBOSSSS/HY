...
# 与 Stage04 默认一致：worldstereo-memory-dmd
NAME=""; NPROC="${NPROC_PER_NODE:-1}"; RESULT_NAME="${HY2_WORLDSTEREO_MODEL_TYPE:-worldstereo-memory-dmd}"
while [[ $# -gt 0 ]]; do
  case "$1" in
    --name) NAME="$2"; shift 2 ;;
...
echo "[run_05_gs_data] result_name=$RESULT_NAME nproc=$NPROC"
